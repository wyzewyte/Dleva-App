...
export const RiderPageShell = ({
  ...
  return (
    <div className={cn('min-h-screen bg-white', withBottomNavSpacing && 'pb-28 md:pb-10', className)}>
      <div className={cn('mx-auto w-full px-4 sm:px-6 md:px-8', maxWidth, contentClassName)}>{children}</div>
    </div>
  );
};

export const RiderPageHeader = ({
  title,
  subtitle,
  action,
  onBack,
    ...
    >
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0">
          <h1 className="text-[1.8rem] font-bold tracking-tight text-dark sm:text-[2rem]">{title}</h1>
          {subtitle ? <p className="mt-1 hidden max-w-2xl text-sm leading-relaxed text-muted lg:block">{subtitle}</p> : null}
        </div>
      </div>
...
export const RiderCard = ({ children, className, interactive = false, muted = false }) => {
  return (
    <div
      className={cn(
...
export const RiderStatusBadge = ({ status, children, className }) => {
  ...
  const styles = {
    online: 'border-emerald-100 bg-emerald-50 text-emerald-700',
    ...
    delivery_attempted: 'border-amber-100 bg-amber-50 text-amber-700',
    verifying: 'border-amber-100 bg-amber-50 text-amber-700',
    offline: 'border-gray-200 bg-gray-100 text-gray-700',
...
export const RiderPrimaryButton = ({ children, className, loading = false, disabled = false, icon, ...props }) => {
  return (
    <button
      className={cn(
...
export const RiderSecondaryButton = ({ children, className, icon, ...props }) => {
  return (
    <button
      className={cn(
        'inline-flex min-h-[46px] w-full items-center justify-center gap-2 rounded-xl border border-gray-200 bg-surface px-4 py-3 text-sm font-bold text-dark transition-all hover:bg-gray-50 active:scale-[0.98]',
        className
      )}
      {...props}
    >
...
export const RiderQuietButton = ({ children, className, icon, ...props }) => {
  return (
    <button
      className={cn(
...
export const RiderSegmentedTabs = ({ tabs, value, onChange, className }) => {
  return (
...
export const RiderFormField = ({ label, hint, error, className, children }) => {
  return (
...
export const RiderFeedbackState = ({ type = 'info', title, message, action, className }) => {
  const config = {
    loading: {
    ...
    info: {
      icon: <Power size={24} className="text-primary" />,
      cardClass: 'border-gray-200 bg-surface',
      titleClass: 'text-dark',
    },
  };
  ...
  if (type === 'loading') {
    return (
      <RiderCard className={cn('p-6', current.cardClass, className)}>
  ...
  return (
    <RiderCard className={cn('p-6 text-center', current.cardClass, className)}>
...
export const RiderEmptyState = ({ icon, title, description, action, secondaryAction, className }) => {
  return (
    <RiderCard className={cn('px-6 py-10 text-center', className)}>
    ...
    <RiderPageShell maxWidth="max-w-md" contentClassName="flex min-h-screen flex-col justify-center py-10">
      <div className={cn('space-y-6', className)}>
        ...
        <div className="text-center">
          {icon ? <div className="mx-auto mb-5 flex items-center justify-center">{icon}</div> : null}
        ...
        <RiderCard className="p-5 sm:p-6">{children}</RiderCard>
